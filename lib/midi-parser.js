...
const MidiMetaMidiPortEvent = types.MidiMetaMidiPortEvent;
...
const MIDI_PORT_META_EVENT = constants.MIDI_PORT_META_EVENT;
...
function mapMidiPortParams(params, dataBytes) {
	params.port = parseByteArrayToNumber(dataBytes); // The last byte is the MIDI port and has values from 0x00 to 0x7F (0 to 127 decimal).
	return params;
}
...
function mapKeySigparams(params, dataBytes) {
	const [ signed ] = new Int8Array([dataBytes[0]]);
	params.sf = signed;
	params.mi = dataBytes[1];
	return params;
}
...
const processMetaEvent = dispatch(
	...
	generateMetaEventGuard(MIDI_PORT_META_EVENT, partial(generateMetaEvent, mapMidiPortParams, MidiMetaMidiPortEvent)),
